...
#define blockSize 1024 //Thread Block size
...
int main(int argc, char **argv) {
  ...
  long *baseVals  = (long*)malloc(longArraySize * sizeof(long));
  int16_t*  isCompressed = (int16_t*)malloc(numBlocks * sizeof(int16_t)) ;
  int32_t*  isCompressedMatrix = (int32_t*)malloc((numBlocks + 1) * sizeof(int32_t)) ;
  char * compressed = (char*)malloc(longArraySize*sizeof(long)) ;
  initArray(numBlocks , isCompressed) ;
  //initArray(numBlocks+1 , isCompressedMatrix);
  const auto start = now() ;
  int bytesCopied = bdCompress((char*)inputArray, longArraySize * sizeof(long) ,compressed ,  isCompressed , baseVals) ;
  const auto end = now() ;
  ...
  char * decompressed = (char*)malloc(sizeof(long)* longArraySize) ;
   ...
   */
   ...
   // std::cout << "De-Compression in GPU time = " << elapsed << " milliseconds.";
    ...
    for (i =0; i < longArraySize * sizeof(long); i++)
    {
