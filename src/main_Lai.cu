...
__global__ void decompress ( char * compressed , char * decompressed , int bytesCopied , long *baseVals , int16_t *isCompressed , int numBlocks)
{
    int i = 0 ;
    ...
    if(tx < numBlocks)
    {
        ...
        if (isCompressed[tx] <0)
        {
          int bytes_to_copy = -1* isCompressed[tx];
        ...
        int chunk_size ;
        int compressed_size ;  
        if((isCompressed[tx] / 4 ) ==1 )
           chunk_size = 2  ;
        else if ((isCompressed[tx]/4) ==2)
           chunk_size = 4 ;
        else 
           chunk_size = 8 ;
